...
def run_server_update(args: argparse.Namespace) -> int:
    ...
    command = render_script_command(root / "scripts" / "linux" / "update_3plug_server.sh", env_vars)
    ...
    job_id = record_server_job(root, args, action="update", summary="Update the installed 3plug CLI on an existing server.", payload=payload)
